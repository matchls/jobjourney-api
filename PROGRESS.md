...
- Extraction IA d'une offre via Groq (V1.2, `matchls/jobjourney-api#17`) — `POST /applications/parse-offer`, **aucune persistance, aucune migration Prisma, aucun changement frontend** :
  ...
  - Prompt système : n'utiliser que l'offre, ne rien inventer, omettre les champs absents, signaler via `uncertainFields`/`warnings`. Le texte de l'annonce est encadré par une balise dont le suffixe est **aléatoire à chaque requête** (`<offre_a3f9...>`), pour qu'une annonce piégée ne puisse pas « fermer » la balise et faire passer la suite pour des instructions
